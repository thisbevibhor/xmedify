...
// Search results page - displays hospitals filtered by state and city
// Reads state/city from URL search params and fetches matching hospitals from API
// Also handles the booking flow with modal and success notification
export default function Search() {
  ...
  // Hardcoded available time slots grouped by time of day
  // These are passed down to HospitalCard for the booking calendar
  ...
  // Fetch hospitals from API whenever state or city changes
  useEffect(() => {
  ...
  // Sync state/city values when URL search params change (e.g. new search from SearchHospital)
  useEffect(() => {
    setState(seachParams.get("state"));
    setCity(seachParams.get("city"));
  }, [seachParams]);

  // Open booking modal with selected hospital details and time slot
